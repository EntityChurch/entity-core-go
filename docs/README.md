...
`architecture/guides/`, and the rolling `STATUS.md`. The repo maintains
additional working tiers locally (`status/`, `validation/`, `reviews/`,
`legacy/`) that are point-in-time or historical and are not part of the
published canonical surface.
...
| `status/` | working | — | Dated, immutable snapshots (`HANDOFF-`, `CHECKPOINT-`, `ROUTING-`, `TRACKER-`, `PEER-PACKET-`). Internal; publishes nothing. |
...
- **Canonical / living:** the root `AGENTS.md` / `README.md`, `cmd/README.md`,
  `STATUS.md`, and `architecture/` (specs, guides). `architecture/` is kept to
  canonical content only — no dated point-in-time files.
