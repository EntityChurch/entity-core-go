...
func cmdStart(args []string) {
	...
	publishPrefix := fs.String("publish-prefix", "", "EXTENSION-TREE §3.3a: the subtree --publish-root commits to, and the `prefix` the published root declares (e.g. system/content/ to publish only shareable content, or / for the universal tree). Empty keeps the peer's own default. The prefix is an upper BOUND on scope, NOT a completeness claim (§3.3a): a publisher MAY declare a broad prefix and publish only a subset under it. Narrowing it (the common deployment) narrows what CAN be published, not which bindings are. Go-only: absent from both sibling CLIs — established by enumerating rust's full `PeerAction::Start` arg set and py's full 49-flag argparse surface, not by grepping for one name — so it is dropped with a warning for those types rather than passed and rejected."+siblingSurfaceVerifiedAt())
